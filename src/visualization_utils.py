...
def plot_icp_iterations_results(iterations_results, save_path):
    ...
    ax[0, 0].plot(error, label='Error')
    ax[0, 0].axvline(x=min_error_index, color='red', linestyle='--', label=f'Minimum Error Index (it = {min_error_index})')
    ax[0, 0].set_title('Error')
    ax[0, 0].set_xlabel('Iteration')
    ax[0, 0].set_ylabel('Error')
    ax[0, 0].set_xticks(x_ticks)
    ax[0, 0].set_xticklabels(x_tick_labels, fontsize=7)
    ax[0, 0].set_yticks(ax[0, 0].get_yticks()[ax[0, 0].get_yticks()>=0])
    ax[0, 0].set_yticklabels(ax[0, 0].get_yticks(), fontsize=7)
    ax[0, 0].grid()
    ax[0, 0].legend()

    ax[0, 1].plot(num_inliers, label='Number of Inliers')
    ax[0, 1].axvline(x=min_error_index, color='red', linestyle='--', label=f'Minimum Error Index (it = {min_error_index})')
    ax[0, 1].set_title('Number of Inliers')
    ax[0, 1].set_xlabel('Iteration')
    ax[0, 1].set_ylabel('Number of Inliers')
    ax[0, 1].set_xticks(x_ticks)
    ax[0, 1].set_xticklabels(x_tick_labels, fontsize=7)
    ax[0, 1].set_yticks([int(y_tick) for y_tick in ax[0, 1].get_yticks()[ax[0, 1].get_yticks()>=0]])
    ax[0, 1].set_yticklabels(ax[0, 1].get_yticks(), fontsize=7)
    ax[0, 1].grid()
    ax[0, 1].legend()

    ax[1, 0].plot(kernel_threshold, label='Kernel Threshold')
    ax[1, 0].axvline(x=min_error_index, color='red', linestyle='--', label=f'Minimum Error Index (it = {min_error_index})')
    ax[1, 0].set_title('Kernel Threshold')
    ax[1, 0].set_xlabel('Iteration')
    ax[1, 0].set_ylabel('Kernel Threshold')
    ax[1, 0].set_xticks(x_ticks)
    ax[1, 0].set_xticklabels(x_tick_labels, fontsize=7)
    ax[1, 0].set_yticks(ax[1, 0].get_yticks()[ax[1, 0].get_yticks()>=0])
    ax[1, 0].set_yticklabels(ax[1, 0].get_yticks(), fontsize=7)
    ax[1, 0].grid()
    ax[1, 0].legend()

    ax[1, 1].plot(dumping_factor, label='Dumping Factor')
    ax[1, 1].axvline(x=min_error_index, color='red', linestyle='--', label=f'Minimum Error Index (it = {min_error_index})')
    ax[1, 1].set_title('Dumping Factor')
    ax[1, 1].set_xlabel('Iteration')
    ax[1, 1].set_ylabel('Dumping Factor')
    ax[1, 1].set_xticks(x_ticks)
    ax[1, 1].set_xticklabels(x_tick_labels, fontsize=7)
    ax[1, 1].set_yticks(ax[1, 1].get_yticks()[ax[1, 1].get_yticks()>=0])
    ax[1, 1].set_yticklabels(ax[1, 1].get_yticks(), fontsize=7)
    ax[1, 1].grid()
    ax[1, 1].legend()
...
def plot_final_results(errors, ratios, save_path):
    ...
    ax[1].plot(translation_errors)
    ax[1].set_title('Translation Errors [m]')
    ...
    ax[2].plot(translation_errors_ratio)
    ax[2].set_title('Translation Errors [ratio]')
